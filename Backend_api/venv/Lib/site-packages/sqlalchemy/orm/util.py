...
class AliasedClass(
    ...
    mapped entity can be used in multiple contexts.   A simple example::
        ...
        session.query(userModel, user_alias).join(
            (user_alias, userModel.id > user_alias.id)
        ).filter(userModel.name == user_alias.name)
...
def with_parent(
    ...
    E.g.::
        ...
        stmt = select(Address).where(with_parent(some_user, userModel.addresses))
    ...
    to indicate the left side of the criteria::
        ...
        a1 = aliased(Address)
        a2 = aliased(Address)
        stmt = select(a1, a2).where(with_parent(u1, userModel.addresses.of_type(a2)))
    ...
    :func:`_orm.with_parent.from_entity` argument::
        ...
        stmt = select(a1, a2).where(
            with_parent(u1, userModel.addresses, from_entity=a2)
        )

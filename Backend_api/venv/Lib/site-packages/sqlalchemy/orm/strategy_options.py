...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def contains_eager(
        ...
        ``userModel`` entity, and the returned ``Order`` objects would have the
        ...
        primary collection of parent objects may already have been loaded::
            ...
            sess.query(userModel).join(userModel.addresses).filter(
                Address.email_address.like("%@aol.com")
            ).options(contains_eager(userModel.addresses)).populate_existing()
    ...
    def load_only(self, *attrs: _AttrType, raiseload: bool = False) -> Self:
        ...
        Example - given a class ``userModel``, load only the ``name`` and
        ``fullname`` attributes::

            session.query(userModel).options(load_only(userModel.name, userModel.fullname))

        Example - given a relationship ``userModel.addresses -> Address``, specify
        subquery loading for the ``userModel.addresses`` collection, but on each
        ``Address`` object load only the ``email_address`` attribute::

            session.query(userModel).options(
                subqueryload(userModel.addresses).load_only(Address.email_address)
            )
        ...
        specifically referred to using the :class:`_orm.Load` constructor::
            ...
            stmt = (
                select(userModel, Address)
                .join(userModel.addresses)
                .options(
                    Load(userModel).load_only(userModel.name, userModel.fullname),
    ...
    def joinedload(
        ...
        examples::
            ...
            # joined-load the "orders" collection on "userModel"
            select(userModel).options(joinedload(userModel.orders))
    ...
    def subqueryload(self, attr: _AttrType) -> Self:
        ...
        examples::
            ...
            # subquery-load the "orders" collection on "userModel"
            select(userModel).options(subqueryload(userModel.orders))
    ...
    def selectinload(
        ...
        examples::
            ...
            # selectin-load the "orders" collection on "userModel"
...
class _LoadElement(
    ...
    def _adjust_effective_path_for_current_path(
        ...
        .. sourcecode:: text
            ...
            lp: userModel -> orders -> Order -> items -> Item -> keywords -> Keyword

            cp: userModel -> orders -> Order -> items
...
class _AttributeStrategyLoad(_LoadElement):
    ...
    e.g.::
        ...
        defer(Order.name)
        selectinload(userModel.orders).lazyload(Order.items)
...
class _TokenStrategyLoad(_LoadElement):
    ...
    e.g.::
        ...
        raiseload("*")
        Load(userModel).lazyload("*")
        defer("*")
        load_only(userModel.name, userModel.email)  # will create a defer('*')
        joinedload(userModel.addresses).raiseload("*")

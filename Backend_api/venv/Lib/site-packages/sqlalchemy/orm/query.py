...
class Query(
    ...
    def __init__(
        ...
        E.g.::
            ...
            q = Query([userModel, Address], session=some_session)

        The above is equivalent to::

            q = some_session.query(userModel, Address)
    ...
    def get(self, ident: _PKIdentityArgument) -> Optional[Any]:
        ...
        E.g.::
            ...
            my_user = session.query(userModel).get(5)
    ...
    def with_entities(
        ...
        e.g.::
            ...
            q = (
                ...
                .join(userModel.address)
                .filter(userModel.name.like("%ed%"))
                .order_by(Address.email)
            )

            # given *only* userModel.id==5, Address.email, and 'q', what
            # would the *next* userModel in the result be ?
            subq = (
                q.with_entities(Address.email)
                .order_by(None)
                .filter(userModel.id == 5)
                .subquery()
            )
        ...
        *,
        ...
        E.g.::
            ...
            q = (
                ...
                .populate_existing()
                .with_for_update(nowait=True, of=userModel)
            )
    ...
    def _filter_by_zero(self) -> Any:
        ...
        # to query(), e.g. query(userModel).from_self() is still in terms of userModel,
    ...
    def join(
        ...
        **Simple Relationship Joins**
        ...
        Consider a mapping between two classes ``userModel`` and ``Address``,
        with a relationship ``userModel.addresses`` representing a collection
        of ``Address`` objects associated with each ``userModel``.   The most
        common usage of :meth:`_query.Query.join`
        is to create a JOIN along this
        relationship, using the ``userModel.addresses`` attribute as an indicator
        for how this should occur::

            q = session.query(userModel).join(userModel.addresses)

        Where above, the call to :meth:`_query.Query.join` along
        ``userModel.addresses`` will result in SQL approximately equivalent to:

        .. sourcecode:: sql

            SELECT user.id, user.name
            FROM user JOIN address ON user.id = address.userID

        In the above example we refer to ``userModel.addresses`` as passed to
        ...
        .. note:: as seen in the above example, **the order in which each
           ...
           ``userModel``, then ``Item``, then ``Order``, in our chain of joins; in
        ...
        **Joins to a Target Entity or Selectable**
        ...
        natural foreign key relationship between two entities::
            ...
            q = session.query(userModel).join(Address)
        ...
        **Joins to a Target with an ON Clause**
        ...
        a SQL expression as the ON clause is as follows::
            ...
            q = session.query(userModel).join(Address, userModel.id == Address.userID)

        The above form may also use a relationship-bound attribute as the
        ON clause as well::

            q = session.query(userModel).join(Address, userModel.addresses)
        ...
        aliases set up using the :func:`~sqlalchemy.orm.aliased` function::
            ...
            q = (
                session.query(userModel)
                .join(a1, userModel.addresses)
                .join(a2, userModel.addresses)
        ...
        equivalent to the one above would be::
            ...
            q = (
                session.query(userModel)
                .join(userModel.addresses.of_type(a1))
                .join(userModel.addresses.of_type(a2))
                .filter(a1.email_address == "ed@foo.com")
                .filter(a2.email_address == "ed@bar.com")
            )
        ...
        **Augmenting Built-in ON Clauses**
        ...
        with the default criteria using AND::
            ...
            q = session.query(userModel).join(
                userModel.addresses.and_(Address.email_address != "foo@bar.com")
            )
        ...
        **Joining to Tables and Subqueries**
        ...
        out of a query::
            ...
            q = session.query(userModel).join(subq, userModel.id == subq.c.userID)
        ...
        entity using :func:`_orm.aliased`::
            ...
            q = session.query(userModel).join(userModel.addresses.of_type(address_subq))
    ...
    def slice(
        ...
        For example, ::
            ...
            session.query(userModel).order_by(userModel.id).slice(1, 3)
    ...
    def column_descriptions(self) -> List[ORMColumnDescription]:
        ...
        Format is a list of dictionaries::
            ...
            user_alias = aliased(userModel, name="user2")
            q = sess.query(userModel, userModel.id, user_alias)
    ...
    def exists(self) -> Exists:
        ...
        e.g.::
            ...
            q = session.query(userModel).filter(userModel.name == "fred")
        ...
        The EXISTS construct is usually used in the WHERE clause::
            ...
            session.query(userModel.id).filter(q.exists()).scalar()
    ...
    def count(self) -> int:
        ...
        expressions in conjunction with :meth:`~.Session.query`, i.e.::
            ...
            # count userModel records, without
            # using a subquery.
            session.query(func.count(userModel.id))

            # return count of user "id" grouped
            # by "name"
            session.query(func.count(userModel.id)).group_by(userModel.name)

            from sqlalchemy import distinct

            # count distinct "name" values
            session.query(func.count(distinct(userModel.name)))
    ...
    def delete(
        ...
        E.g.::
            ...
            sess.query(userModel).filter(userModel.age == 25).delete(synchronize_session=False)

            sess.query(userModel).filter(userModel.age == 25).delete(
    ...
    def update(
        ...
        E.g.::
            ...
            sess.query(userModel).filter(userModel.age == 25).update(
                {userModel.age: userModel.age - 10}, synchronize_session=False
            )

            sess.query(userModel).filter(userModel.age == 25).update(
                {"age": userModel.age - 10}, synchronize_session="evaluate"
            )

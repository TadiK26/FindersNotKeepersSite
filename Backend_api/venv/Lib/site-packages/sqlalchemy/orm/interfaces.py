...
class MapperProperty(
    ...
    def class_attribute(self) -> InstrumentedAttribute[_T]:
        ...
        and the class to which it is mapped is ``userModel``, this sequence
        is possible::

            >>> from sqlalchemy import inspect
            >>> mapper = inspect(userModel)
            >>> addresses_property = mapper.attrs.addresses
            >>> addresses_property.class_attribute is userModel.addresses
            True
            >>> userModel.addresses.property is addresses_property
            True
...
class PropComparator(SQLORMOperations[_T_co], Generic[_T_co], ColumnOperators):
    ...
    userModel-defined subclasses of :class:`.PropComparator` may be created. The
    ...
    def and_(
        ...
        E.g.::
            ...
            stmt = select(userModel).join(
                userModel.addresses.and_(Address.email_address != "foo")
            )

            stmt = select(userModel).options(
                joinedload(userModel.addresses.and_(Address.email_address != "foo"))
            )

...
class Mapper(
    ...
    def __init__(
        ...
        :param column_prefix: A string which will be prepended
           ...
           ``userID``, ``user_name``, and ``password``::

                class userModel(Base):
                    __table__ = user_table
                    __mapper_args__ = {"column_prefix": "_"}

           The above mapping will assign the ``userID``, ``user_name``, and
           ``password`` columns to attributes named ``_user_id``,
           ``_user_name``, and ``_password`` on the mapped ``userModel`` class.
        ...
        if (
            ...
            if warn_for_existing:
                util.warn_deprecated(
                    f"userModel-placed attribute {self.class_.__name__}.{key} on "
    ...
    def attrs(self) -> util.ReadOnlyProperties[MapperProperty[Any]]:
        ...
        ``userModel`` class which has ``userModel.name`` attribute would

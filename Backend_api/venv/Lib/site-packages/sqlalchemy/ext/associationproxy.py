...
class AssociationProxy(
    ...
    def for_class(
        ...
        target class for this proxy as mapped by ``userModel``::

            inspect(userModel).all_orm_descriptors["keywords"].for_class(userModel).target_class

        This returns an instance of :class:`.AssociationProxyInstance` that
        is specific to the ``userModel`` class.   The :class:`.AssociationProxy`
...
class AssociationProxyInstance(SQLORMOperations[_T]):
    ...
    :meth:`.AssociationProxy.for_class` method::
        ...
        proxy_state = inspect(userModel).all_orm_descriptors["keywords"].for_class(userModel)

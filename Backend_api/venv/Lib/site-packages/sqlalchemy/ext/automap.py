...
asking it to reflect the schema and produce mappings::
    ...
    session.add(Address(email_address="foo@bar.com", user=userModel(name="foo")))
    session.commit()

    # collection-based relationships are by default named
    # "<classname>_collection"
    u1 = session.query(userModel).first()
...
our basic mapping consisting of related ``userModel`` and ``Address`` classes is
...
``user`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare userModel for the 'user' table
    ...
    # but userModel is the same as Base.classes.userModel now
...
``userModel.address_collection`` and the name of the class referred to, from
...
From the above mapping, we would now have classes ``userModel`` and ``Address``,
where the collection from ``userModel`` to ``Address`` is called
``userModel.addresses``::

    userModel, Address = Base.classes.userModel, Base.classes.Address

    u1 = userModel(addresses=[Address(email="foo@bar.com")])
...
defines table metadata::
    ...
    a1 = Address(email="u1")
    a2 = Address(email="u2")
    u1 = userModel(address_collection=[a1, a2])
    assert a1.user is u1

Above, given mostly complete ``userModel`` and ``Address`` mappings, the
:class:`_schema.ForeignKey` which we defined on ``Address.userID`` allowed a
bidirectional relationship pair ``Address.user`` and
``userModel.address_collection`` to be generated on the mapped classes.
...
class AutomapBase:
    ...
    :attr:`.AutomapBase.by_module` looks like::
        ...
        userModel = Base.by_module.sqlalchemy.ext.automap.userModel

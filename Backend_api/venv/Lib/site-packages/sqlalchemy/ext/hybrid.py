...
mapping which relates a ``userModel`` to a ``SavingsAccount``::
...
.. tip:: The ``userModel.balance`` getter in the above example accesses the
   ``self.acccounts`` collection, which will normally be loaded via the
   :func:`.selectinload` loader strategy configured on the ``userModel.balance``
...
At the expression level, it's expected that the ``userModel`` class will
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select
    >>> print(
    ...     select(userModel, userModel.balance)
    ...     .join(userModel.accounts)
    ...     .filter(userModel.balance > 5000)
    ... )
    {printsql}SELECT "user".id AS userID, "user".name AS user_name,
    account.balance AS account_balance
    FROM "user" JOIN account ON "user".id = account.userID
...
.. sourcecode:: pycon+sql
    ...
    >>> print(
    ...     select(userModel, userModel.balance)
    ...     .outerjoin(userModel.accounts)
    ...     .filter(or_(userModel.balance < 5000, userModel.balance == None))
    ... )
    {printsql}SELECT "user".id AS userID, "user".name AS user_name,
    account.balance AS account_balance
    FROM "user" LEFT OUTER JOIN account ON "user".id = account.userID
    ...
    class userModel(Base):
        ...
        def _balance_expression(cls) -> SQLColumnExpression[Decimal]:
            return (
                ...
                .where(SavingsAccount.userID == cls.id)
                .label("total_balance")
            )
...
.. sourcecode:: pycon+sql
    ...
    >>> from sqlalchemy import select
    >>> print(select(userModel).filter(userModel.balance > 400))

...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def define_reflected_tables(cls, metadata, schema):
        ...
        if testing.requires.self_referential_foreign_keys.enabled:
            parent_id_args = (
                ForeignKey(
                    "%susers.userID" % schema_prefix, name="user_id_fk"
                ),
            )
        else:
            parent_id_args = ()
        users = Table(
            "users",
            metadata,
            Column("userID", sa.INT, primary_key=True),
        ...
        Table(
            ...
            Column(
                "id_user",
                sa.Integer,
                ForeignKey("%susers.userID" % schema_prefix),
            ),
        ...
        Table(
            ...
            Column("remote_user_id", sa.Integer, ForeignKey(users.c.userID)),
        ...
        if testing.requires.index_reflection.enabled:
            ...
            Index(
                "users_all_idx", users.c.userID, users.c.test2, users.c.test1
            )
    ...
    def exp_columns(
        ...
        views = {
            ...
            (schema, "users_v"): [
                col("userID", auto="omit", nullable=mock.ANY),
    ...
    def exp_fks(
        ...
        tables = {
            (schema, "users"): [
                fk(["parent_user_id"], ["userID"], "users", name="user_id_fk")
            ],
            (schema, "dingalings"): [
                fk(["id_user"], ["userID"], "users"),
                fk(
                    ["address_id"],
                    ["address_id"],
                    "email_addresses",
            ...
            (schema, "email_addresses"): [
                fk(["remote_user_id"], ["userID"], "users")
            ],
            (schema, "comment_test"): [],
            (schema, "no_constraints"): [],
            (schema, "local_table"): [
    ...
    def exp_indexes(
        ...
        tables = {
            (schema, "users"): [
                ...
                *idx("userID", "test2", "test1", name="users_all_idx"),
    ...
    def test_get_foreign_keys(self, connection, use_schema):
        ...
        if testing.requires.self_referential_foreign_keys.enabled:
            ...
            eq_(fkey1["referred_columns"], ["userID"])

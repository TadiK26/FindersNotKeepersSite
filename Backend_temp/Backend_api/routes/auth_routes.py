...
def login():
    ...
    start_user_session(user)  # ✅ Store user info in session
...
#     jti=get_jwt()["jti"]
#     revoke_token.add(jti)
#     clear_user_session()  # ✅ clear Flask session
...
#     revoke_token.add(jti)
#     log_audit(userID=user_id, action="LOGOUT", details=f"Access token revoked from {request.remote_addr}")
#     return jsonify({"message": "Access token revoked, user logged out successfully."}), 200

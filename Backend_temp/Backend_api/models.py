...
class AuditLog(db.Model):
    __tablename__ = "Auditlog"  # use lowercase for consistency
    logID = db.Column(db.Integer, primary_key=True, autoincrement=True)
    userID = db.Column(db.Integer, nullable=False)
    action = db.Column(db.String(255), nullable=False)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
...
class Listings(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users_table.userID'))  # <-- match your user table
...
class SessionLog(db.Model):
    ...
    logout_time = db.Column(db.DateTime, nullable=True)  # None until logout
